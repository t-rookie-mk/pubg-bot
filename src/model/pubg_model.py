...
import time
import numpy as np
from model.mouse_model import HIDMouse, WindowApiMouse
...
class PubgModel:
    ...
    def __init__(self) -> None:
        ...
        self.machine.add_transition(trigger='load', source='mathching', dest='loading_hall',
                                    conditions=['check_loading'])
        self.machine.add_transition(trigger='mark', source='loading_hall', dest='loading_hall_ok',
                                    conditions=['check_marking'], prepare=['marking'])
        self.machine.add_transition(trigger='plane', source='loading_hall_ok', dest='plane', conditions=['check_plane'],
                                    after='plane_find_pos')
        self.machine.add_transition(trigger='ground', source='plane', dest='ground', conditions=['check_ground'],
                                    after='goto_bp')

        self.machine.add_transition(trigger='end', source='*', dest='end', conditions=['check_end'],
                                    after='return_lobby')

        self.machine.add_transition(trigger='error', source='*', dest='error', conditions=['check_error'],
                                    after='mix_error')
        # 获取窗口大小
        ...
        self.error_image_list = [self.pic_dict['error'], self.pic_dict['error1'], self.pic_dict['error2'],
                                 self.pic_dict['error3']]

        self.mouse = WindowApiMouse()
        pass
    ...
    def match(self) -> bool:
        ...
        if x is None:
            return False
        self.mouse.move_to(int(x), int(y))
        self.mouse.click()
        time.sleep(1)
        x, y = self.image_finder.find_image_in_screen(self.pic_dict['start_match'])
        return x is None
    ...
    def pick(self):
        ...
        self.mouse.move_to(150 + self.coord[0], 120 + self.coord[1])
    ...
    def mix_error(self):
        ...
        if x is not None:
            ...
            self.mouse.click()
            time.sleep(1)
            self.mouse.move_to(150 + self.coord[0], 540 + self.coord[1])
            time.sleep(1)
            self.mouse.click()
            time.sleep(1)
    ...
    def mark_map_pos_first(self, relative_x, relative_y):
        ...
        # 清除之前的标记
        self.mouse.move_to(self.coord[0] + 500, self.coord[1] + 100)
        time.sleep(0.5)
    ...
    def random_movement(self):
        # 随机选择一个方向键
        direction = random.choice(['w', 'a', 'd'])
        # 随机按下和释放时间
        press_time = random.uniform(3, 10)
